...
if(isset($_POST)) {
	...
	$confirm_password = mysqli_real_escape_string($conn, $_POST['confirmpas']);
	...
	if($result->num_rows == 0) {
		...
		$sql = "INSERT INTO employee(first_name, last_name, email, password, confirmpas) VALUES ( $firstname', '$lastname', '$email', '$password', '$confirm_pass')";

		if($conn->query($sql)===TRUE) {

			//If data inserted successfully then Set some session variables for easy reference and redirect to company login
			$_SESSION['registerCompleted'] = true;
			header("Location: welcomemp.php");
			exit();
