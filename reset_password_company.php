...
if (isset($_POST['submit'])) {
    ...
    if ($password === $confirmPassword) {
        ...
        $sql = "SELECT * FROM res_pass WHERE token='$token'";
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            $sql = "UPDATE comapny SET password='$hashedPassword' WHERE email='$email'";
            mysqli_query($conn, $sql);

            // Delete the token from the res_password table
            $sql = "DELETE FROM res_pass WHERE token='$token'";
            mysqli_query($conn, $sql);
            ...
            header("Location: forget_password_company.php");
            exit();
        }
    } else {
        $_SESSION['error'] = "Password and confirm password do not match.";
        header("Location: reset_password_company.php?token=" . urlencode($token));
        exit();
    }
} else {
